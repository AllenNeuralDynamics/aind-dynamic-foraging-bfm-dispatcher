"""Freeze the paired current-code E=4/E=8 diagnostic transfer launches."""
...
VARIANT = STUDY / "variants" / "gru-e8-d614-diagnostic"
VALIDATION = STUDY / "analysis" / "dataset_suite_validation.json"
HISTORICAL = STUDY / "analysis" / "matched_half_results.json"
CACHE = STUDY / "analysis" / "_cache_embedding_dimension"
OUTPUT = STUDY / "analysis" / "embedding_dimension_results.json"


def _launch(dimension: int) -> tuple[str, str]:
    root = VARIANT / f"launch_record_e{dimension}"
    ...
    if len(spec["tasks"]) != 15 or len(groups) != 1:
        raise AssertionError(f"E={dimension} launch is not one complete 15-cell group")
    return groups.pop(), record["experiment_id"]


def _manifest(dimension: int) -> tuple[Path, dict, str]:
    path = STUDY / f"source_runs_e{dimension if dimension == 8 else '4_pair'}.json"
    raw = path.read_bytes()
    manifest = json.loads(raw)
    if manifest.get("status") != "complete":
        raise AssertionError(f"{path.name} is not complete")
    if manifest["architecture"]["subject_embedding_size"] != dimension:
        raise AssertionError(f"{path.name} declares the wrong embedding dimension")
    expected = {f"e{dimension}-d614-s{seed}" for seed in range(3)}
    if set(manifest["runs"]) != expected:
        raise AssertionError(f"{path.name} does not contain exactly three source seeds")
    return path, manifest, hashlib.sha256(raw).hexdigest()


def _freeze_dimension(dimension: int) -> tuple[str, str, dict[str, list[dict]]]:
    group, experiment_id = _launch(dimension)
    manifest_path, manifest, manifest_digest = _manifest(dimension)
    runs = _wandb_runs(group)
    tasks = _beaker_tasks(experiment_id)
    if len(tasks) != 15 or len(runs) != 15:
        raise AssertionError(f"E={dimension} must have 15 Beaker tasks and 15 W&B runs")
    records = {name: [] for name in DATASET_ORDER}
    for task in tasks:
        job = task["jobs"][-1]
        status = job["status"]
        ...
        if source.get("manifest") != manifest_path.name:
        ...
        files = _cached_wandb_report_files(
            artifact, CACHE / f"e{dimension}" / dataset / source_key
        )
    ...
    for dataset, rows in records.items():
        ...
        if [row["seed"] for row in rows] != [0, 1, 2]:
            raise AssertionError(f"E={dimension} {dataset} lacks three source seeds")
    return group, experiment_id, records


def main() -> None:
    e4_group, e4_experiment, e4 = _freeze_dimension(4)
    e8_group, e8_experiment, e8 = _freeze_dimension(8)
    validation = {
        row["dataset"]: row
    ...
    for name in DATASET_ORDER:
        trial_signatures = {
            ...
            for row in e4[name] + e8[name]
        }
        old_d614 = [
            row for row in historical[name]["gru"] if int(row["nominal_D"]) == 614
        ]
        trial_signatures.update(
            (row["ordered_trial_key_sha256"], row["n_prediction_rows"])
            for row in old_d614
        )
        if len(trial_signatures) != 1:
            raise AssertionError(f"E=4/E=8/historical trial keys differ for {name}")
        ...
        datasets[name] = {
            ...
            "ordered_trial_key_sha256": next(iter(trial_signatures))[0],
            "e4": e4[name],
            "e8": e8[name],
        }
    output = {
        ...
        "contract": {
            ...
            "exact_trial_key_parity": True,
        },
        "wandb_project": f"https://wandb.ai/{ENTITY}/{PROJECT}",
        "beaker_experiments": {"e4": e4_experiment, "e8": e8_experiment},
        "datasets": datasets,
    }
