# Eckstein (human) RL co-winner

Fit Eckstein et al.'s four-parameter asymmetric, counterfactual RL model on each
participant's complete first-half prefix. Replay that prefix with the fitted
parameters to initialize values, then score only the immutable second-half
suffix used by GRU and common Q.
