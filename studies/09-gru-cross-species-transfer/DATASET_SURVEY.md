...
| Tang (macaque) | Macaque; blockwise action/object values | [Mendeley Data v1](https://doi.org/10.17632/m4f38w49fb.1), CC BY 4.0 | 2 | 8 | 15,375 | 7,728 | v1 | feedback-dependent Rescorla-Wagner with separate parameters for What and Where blocks | Moderate; equations are published, but initialization and exact optimizer settings are not | Task-aware author baseline completed; held-out likelihood 0.616669 with exact trial-key parity |
| Alsiö (rat) | Rat; visual/spatial discrimination and reversal | [Cambridge Apollo](https://doi.org/10.17863/CAM.80290), CC BY 4.0 | 95 | 2,334 | 457,007 | 225,751 | v1 | separate positive/negative learning-rate RL was selected for the separate PRL cohort | High for direct comparison: selected model and admitted cohort do not align | Cohort-mismatched author-model sensitivity completed; held-out likelihood 0.527446 with exact trial-key parity |
...
| Costa (macaque) | Macaque; stochastic stimulus reversal | [Zenodo record 20086410](https://doi.org/10.5281/zenodo.20086410), CC BY 4.0 | 11 | 245 | 329,840 | 162,960 | v1 | feedback-dependent RL with separate positive/negative learning rates | Moderate; equations are published, original fitting code not found | Completed after reward-column correction; corrected E4, E8, Bari2019, and author-model fits have exact held-out trial-key parity |
| López-Yépez (mouse) | Mouse; baited variable-interval dynamic matching | [Figshare v1](https://doi.org/10.6084/m9.figshare.14540283.v1), CC BY 4.0 | 8 | 218 | 147,726 | 77,662 | v1 | double-trace RL with fast and slow choice traces | Moderate; equations are published, MATLAB implementation was not released with the data | Completed; E4, E8, Bari2019, and double-trace author-model fits have exact held-out trial-key parity |
| Hattori (mouse) | Mouse; baited probabilistic reversals during longitudinal OFC imaging | [Zenodo v3](https://doi.org/10.5281/zenodo.10969434) (no explicit Zenodo data license) | 7 | 292 | 141,488 | 68,972 | v1 | asymmetric rewarded/unrewarded Q-learning with unchosen-value forgetting | Low; the exact equations match the existing `Hattori2019` family | Completed mature-only; paper-defined day-15 cutoff, checksum, exact-count, deterministic-manifest, wrapper-loader, matched-trial, E4, E8, Bari2019, and Hattori2019 audits passed |
...
Costa (macaque) uses the released coding template to disambiguate adjacent MATLAB
columns: released column 9 is the choice's phase-relative correct-side code, column 10
is acquisition/reversal phase, and column 11 is rewarded/unrewarded choice feedback.
Canonical `rewarded` therefore comes from column 11. An earlier adapter used column 9;
all results derived from that mapping were invalidated, and the corrected immutable
table keeps the same subjects, sessions, split manifest, and held-out trial identities.
...
| Tang (macaque) | feedback-dependent Rescorla-Wagner, separately fit to What and Where blocks | Published equations; neural-analysis code does not include the behavioral fit | Object values must be learned in What blocks and action values in Where blocks; initialization and exact optimizer settings are not reported | Implemented; matched-half fit completed | High for equations and domain mapping; moderate for paper-level fitting parity |
| Alsiö (rat) | separate positive/negative-rate RL plus spatial stickiness | Paper equations; no pinned package | Selected model was evaluated on excluded PRL cohort VI, not admitted II–V cohorts | Implemented; matched-half sensitivity completed | High for equations; low for a direct author-aligned claim on the admitted cohort |
...
| Costa (macaque) | feedback-dependent RL | Published equations | Matched-half subject fitting replaces the paper's phase-specific fitting protocol | Implemented; corrected matched-half fit completed | High for equations; moderate for paper-level fitting parity |
| López-Yépez (mouse) | double-trace RL | Published MATLAB equations | Original data-fit code and initialization details are not in the release | Implemented; matched-half fit completed | High for equations; moderate for paper-level fitting parity |
...
The primary set passed this gate first. Tang (macaque), Alsiö (rat), Costa
(macaque), and López-Yépez (mouse) were then explicitly selected for the second
round. Alsiö (rat) remains labeled as a cohort-mismatched sensitivity rather than a
direct author-aligned reproduction.
